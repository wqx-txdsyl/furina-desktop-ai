...
> 依据 `PRD.md` + `plan/0~8`。**修正**：文档中的 “Qwen3.8” 实际为 **智谱 GLM**（`.env` 的 `ZHIPU_API_KEY`；默认 `glm-4v-flash`，免费，视觉+对话，已实测能识别基座图）。
> `plan/8` 对 “Qwen 的要求” 是**写代码的工程约束**，作为开发铁律遵守。
> 资产由 **Agnes AI**（`agnes-image-2.1-flash` 图 / `agnes-video-v2.0` 视频）基于 `furina-base.png` 生成，当前 $0。

## 里程碑：先最小垂直切片，再扩展（plan/8 §“build smallest working vertical slice first”）
