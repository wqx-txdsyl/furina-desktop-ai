...
- ``broker``：ApprovalBroker（状态所有者——exactly-once 决议、超时、撤销、会话
  grant、**Patch 3：单锁原子消费（全部校验后单点提交）**、redacted 域事件；owner
  只在构造时绑定；approve_session/grant 消费时刻重查可信 USER 记录并要求
  **typed context 完全相等**）+ PermitIssuer（**Patch 3：permit 签发器——内部绑定
  唯一 gate_id + expected contract_id/hash，仅决策面（owner 线程）
  ``create_permit_issuer`` 创建；公开 GateSeal/issue_permit 已删除，producer 面
  零签发能力**）；
