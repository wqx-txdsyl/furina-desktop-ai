"""NIGHT03_PATCH2B_R1A_R5 — negative mutation + reproducibility tests.

Every test really mutates a temporary copy of the committed data and
runs the independent verifier
(scripts/assets_v2/night03_patch2b_r1a_r5_verify_plan.py) as a
subprocess.  The verifier must REJECT every mutation (exit != 0) and
accept the pristine delivery (exit == 0).

Where the task book demands it, the manifest is re-synced after the
tamper so the rejection provably comes from the semantic/pixel/report
cross-checks, not from stale-hash detection.
...
Covered mutations:
  ...
  5.  a16 provenance section deleted                  -> FAIL
  6.  a16 control-row/part claims re-injected         -> FAIL
  7.  a01 tail raw source over alpha<=8               -> FAIL
  8.  a01 tail raw source hits protected              -> FAIL
  9.  a05 tail raw source hits protected              -> FAIL
  10. tail duplicate/overlapping run                  -> FAIL
  11. tail out-of-bounds / negative coordinates       -> FAIL
  12. tail source_px metadata tamper                  -> FAIL
  13. tail review UNREVIEWED flag tamper              -> FAIL
  14. mirror destination outside tail map             -> FAIL
  15. R6A-R4 primitive/ledger tampering               -> FAIL
  16. R4 protected mask tampering                     -> FAIL
  17. cutout 1px tamper (manifest re-synced)          -> FAIL
  18. overlay 1px tamper (manifest re-synced)         -> FAIL
  19. zoom evidence 1px tamper (manifest re-synced)   -> FAIL
  20. indexed palette tamper (manifest re-synced)     -> FAIL
  21. stale report (manifest re-synced)               -> FAIL
  22. stale manifest                                  -> FAIL
...
VERIFIER = REPO / ('scripts/assets_v2/'
                   'night03_patch2b_r1a_r5_verify_plan.py')
FREEZE = REPO / ('scripts/assets_v2/'
                 'night03_patch2b_r1a_r5_freeze_plan.py')
SOURCES = REPO / 'data/assets_v2/annotation_sources/night03_patch2b_r1a_r5'
...
def resync_manifest(out_dir, rel):
    """Re-hash `rel` into the manifest so stale-hash detection cannot
...
def test_tail_duplicate_run_rejected(tmp_path):
    src = copy_sources(tmp_path)
    doc = load_json(src / 'a01_tail_source.json')
    doc['rows'].append([1000, [200, 210], [205, 215]])
    save_json(src / 'a01_tail_source.json', doc)
    r = run_verifier(sources=src)
    assert r.returncode != 0, 'tail with overlapping runs accepted'


def test_tail_out_of_bounds_and_negative_rejected(tmp_path):
    src = copy_sources(tmp_path)
    doc = load_json(src / 'a01_tail_source.json')
    doc['rows'].append([2000, [0, 5]])
    save_json(src / 'a01_tail_source.json', doc)
    r = run_verifier(sources=src)
    assert r.returncode != 0, 'tail with out-of-bounds row accepted'
    doc = load_json(src / 'a01_tail_source.json')
    doc['rows'] = doc['rows'][:-1]
    doc['rows'].append([100, [[-5, 10]]])
    save_json(src / 'a01_tail_source.json', doc)
    r = run_verifier(sources=src)
    assert r.returncode != 0, 'tail with negative coordinate accepted'


def test_tail_source_px_tamper_rejected(tmp_path):
    src = copy_sources(tmp_path)
    doc = load_json(src / 'a01_tail_source.json')
    doc['source_px'] = doc['source_px'] - 1
    save_json(src / 'a01_tail_source.json', doc)
    r = run_verifier(sources=src)
    assert r.returncode != 0, 'tampered tail source_px accepted'


def test_tail_unreviewed_flag_tamper_rejected(tmp_path):
    src = copy_sources(tmp_path)
    doc = load_json(src / 'a05_tail_source.json')
    doc['review']['unreviewed_px'] = 1
    save_json(src / 'a05_tail_source.json', doc)
    r = run_verifier(sources=src)
    assert r.returncode != 0, 'tail claiming UNREVIEWED>0 accepted'
...
def test_stale_report_with_manifest_sync_rejected(tmp_path):
    out = copy_output(tmp_path)
    rel = 'NIGHT03_PATCH2B_R1A_R5_REPORT.md'
    p = out / rel
...
def test_fresh_checkout_byte_reproducibility(tmp_path):
    """fresh exact checkout -> delete R1A-R5 output -> single freeze
    ...
    rel_out = Path('data/assets_v2/repair_candidates/night03_patch2b_r1a_r5')
    committed = ref / rel_out
    assert committed.exists()
    shutil.rmtree(work / rel_out)          # delete output in the worktree
    assert not (work / rel_out).exists()

    def freeze_and_compare(label):
        r = subprocess.run([sys.executable, str(work / FREEZE.relative_to(REPO))],
