...
def test_1_real_relationship_initial_not_all_guarded():
    ...
    cases = {
        ...
        "action": "帮我打开记事本。",
        "tease": "怎么，不服？",
        "focalors": "你和芙卡洛斯是什么关系？",
    }
    modes = {k: plan_for(v, trust=0.0, familiarity=0.0, annoyance=0.0).mode
             for k, v in cases.items()}
    assert modes["correction"] == "SINCERE"
    assert modes["confide"] == "SINCERE"
    assert modes["quiet"] == "SINCERE"
    assert modes["action"] == "RESPONSIBLE"
...
def test_12_current_vs_recent_activity_recovery():
    ...
    app._current_activity_truth = "read"
    app._recent_activity = "explore"
    app._recent_activity_finished_at = time.time()
    ...
    snap_current = DialogueContextSnapshot(user_text="你现在在干嘛？", activity="read",
                                           channel="DIRECT_USER_TURN", user_initiated=True)
    r_recent = app._grounded_fact_recovery(snap_recent, res)
    r_current = app._grounded_fact_recovery(snap_current, res)
    assert "四处走走" in r_recent, f"问刚才 → recent（explore）: {r_recent}"
    assert "看书" in r_current, f"问现在 → current（read）: {r_current}"
