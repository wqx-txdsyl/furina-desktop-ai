...
用法：
    python scripts/assets/generate_assets.py --dry-run            # 只看要生成什么
    python scripts/assets/generate_assets.py                      # 真实生成（当前 $0）
    python scripts/assets/generate_assets.py --only sit_happy     # 只生成指定语义
"""
...
log = get_logger("scripts.assets.generate_assets")
...
def run_qc(cfg, out: str) -> int:
    """视觉+自动质检（legacy-plan/2 §24-25）：identity/anatomy/style/transparency。"""
