"""NIGHT-03 Recovery Patch 2A R5 — verify the ownership map + masks (independent).

R5 checks per the Reviewer Gate:
- verifier root points ONLY at night03_patch2a_r5; candidate scan scans R5;
...
R5 = ROOT / 'data/assets_v2/repair_candidates/night03_patch2a_r5'
PLAN_DIR = R5 / 'mask_plan'
REPORT_MD = R5 / 'NIGHT03_PATCH2A_R5_MASK_FREEZE_REPORT.md'
RECEIPT_TXT = R5 / 'NIGHT03_PATCH2A_R5_RECEIPT.txt'
...
def main():
    ...
    for asset, ent in plan['assets'].items():
        ...
        # byte-identity vs R3: a01/a05 only (a16 was reworked in R5)
        if asset in ('a01', 'a05'):
            for name, meta in ent['masks'].items():
    ...
    if REPORT_MD.exists():
        ...
        if 'STATUS = READY_FOR_NIGHT03_PATCH2A_R5_OWNERSHIP_REVIEW' not in txt:
    ...
    if RECEIPT_TXT.exists():
        ...
        for k in ('A01_MASKS_BYTE_IDENTICAL_TO_R4', 'A05_MASKS_BYTE_IDENTICAL',
                  'OWNERSHIP_MAP_SINGLE_SOURCE', 'VERIFIER_SCANS_R5',
                  'MASTERS_UNCHANGED'):
            if pairs.get(k) != 'true':
    ...
    # ---- no-candidate scan on R5 + self-tests ----
    probe = R5 / 'mask_plan' / 'a16' / 'a16_fake_candidate.png'
    Image.new('L', (4, 4)).save(probe)
    caught = False
    for p in R5.rglob('*.png'):
        rel = p.relative_to(R5).as_posix()
        nm = p.name.lower()
    ...
    # ---- R5 ownership map checks ----
    own_p = R5 / 'mask_plan' / 'a16_ownership_map.npy'
    if not own_p.exists():
        fail(msgs, 'ownership map npy missing')
    else:
        own = np.load(own_p)
        vals = np.unique(own)
        if not np.isin(vals, list(range(10))).all():
            fail(msgs, f'ownership labels outside 0..9: {vals}')
        a16e = plan['assets']['a16']
        un = a16e.get('ownership_unassigned_alpha_px', -1)
        mf = ROOT / a16e['master_file']
        am = np.array(Image.open(mf).convert('RGBA'))[..., 3] > 8
        actual = int(((own == 7) & am).sum())
        if actual != un:
            fail(msgs, f'ownership unassigned px mismatch: {actual} != {un}')
        eqmap = {'cane_removal': 5, 'chair_removal': 6,
                 'protected_existing_hair': 1, 'protected_quill_paper': 2,
                 'protected_glove_gold_cuff': 3}
        for nm, lb in eqmap.items():
            pp = PLAN_DIR / 'a16' / f'a16_{nm}.png'
            if not pp.exists():
                continue
            mm = np.array(Image.open(pp)) == 255
            if not ((own == lb) == mm).all():
                fail(msgs, f'a16/{nm}: not equal to ownership label {lb}')
    ...
    print('  self-tests passed; zero candidates; R5 dir scanned.')
    sys.exit(0)
