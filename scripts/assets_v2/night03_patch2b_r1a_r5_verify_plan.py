"""NIGHT-03 Patch 2B R1A-R5 — INDEPENDENT plan verifier.

Shares no code with night03_patch2b_r1a_r5_freeze_plan.py.  Re-derives
...
R1A-R5 additions over R1A-R4:
  * tail source: strict row validation (bounds / strict order /
    no overlap / no duplicates), source_px == reconstructed pixels,
    format / canvas / alpha_threshold / adjudication metadata checks,
    and FULL review-record validation (draft == accepted | rejected,
    accepted == final rows, disjoint sets, reason vocabulary,
    totals, UNREVIEWED == 0).
  * a16 provenance contract (rows are the sole authority; no
    control-row / part claims).
  * cutouts, colour partition, semantic overlay and all eight zoom
    evidence PNGs are rebuilt pixel-by-pixel and compared.
  * indexed partition: index values, palette colours and the
    transparency index are verified.
"""
...
STATUS_EXPECTED = 'READY_FOR_NIGHT03_PATCH2B_R1A_R5_PLAN_REVIEW'
...
def validate_row_spec_static(row, h, w, errs, what):
    """Independent strict row validation. Returns list of error strings."""
    ...
    for run in row[1:]:
        if not (isinstance(run, list) and len(run) >= 2):
            errs.append(f'{what}: malformed run')
            return
        a, b = run[0], run[1]
        for v in (a, b):
            if not isinstance(v, int) or isinstance(v, bool):
                errs.append(f'{what}: non-int coord {v!r}')
                return
        if a < 0 or b < a or b >= w:
            errs.append(f'{what}: run {a}-{b} out of bounds')
            return
        if a <= prev_end:
            errs.append(f'{what}: unsorted/overlapping runs at y={y}')
            return
        prev_end = b
...
def verify(sources, output, r6ar4_json, r4masks, masters):
    ...
    for row in occ.get('rows', []):
        validate_row_spec_static(row, H, W, errs, 'a16 rows')
        if errs:
            break
        y = row[0]
        for a, b, cname in row[1:]:
            if cname not in CLASSES:
                errs.append(f'unknown class {cname!r}')
                break
            span = sem[y, a:b + 1]
            if (span != -1).any():
                errs.append(f'duplicate px y={y} x{a}-{b}')
                break
            sem[y, a:b + 1] = CLASSES.index(cname)
        if errs:
            break
    ck.check(not errs, f'a16 rows strict validation ({errs[:1]})')
    if errs:
        raise VerifyFail(f'a16 source rows invalid: {errs[:3]}')
    labeled = (sem != -1) & union
    ...
    if ck.check(colp_p.exists() and ov_p.exists(),
        ...
        colp = (m16[..., :3].astype(np.float32)
                * (m16[..., 3:4].astype(np.float32) / 255.0)
                + bgc * (1 - m16[..., 3:4].astype(np.float32) / 255.0))
        for i, cname in enumerate(CLASSES):
            m = (sem == i) & union
    ...
    for asset in ('a01', 'a05'):
        ...
        errs2 = []
        for row in doc_t.get('rows', []):
            validate_row_spec_static(row, h, w, errs2, f'{asset} rows')
        ck.check(not errs2,
                 f'{asset} rows strict validation ({errs2[:1]})')
        if errs2:
            raise VerifyFail(f'{asset} rows invalid: {errs2[:3]}')
        ...
        if ck.check(isinstance(review, dict), f'{asset} review record dict'):
            ...
            ck.check(review.get('unreviewed_px') == 0,
                     f'{asset} UNREVIEWED == 0')
            ...
            if ck.check(isinstance(rrows, list), f'{asset} review rows'):
                ...
                for entry in rrows:
                    ...
                    y, d_r, a_r, r_r = entry
                    e3 = []
                    validate_row_spec_static([y] + list(d_r), h, w, e3,
                                             f'{asset} review')
                    validate_row_spec_static([y] + list(a_r), h, w, e3,
                                             f'{asset} review')
                    for run in r_r:
                        if not (isinstance(run, list) and len(run) == 3):
                            ok_sets = False
                            break
                        validate_row_spec_static(
                            [y] + [[run[0], run[1]]], h, w, e3,
                            f'{asset} review')
                    ...
                    if not np.array_equal(dm, am | rm) or (am & rm).any() \
                            or not np.array_equal(am, final_rows_mask[y]):
                        ok_sets = False
                        break
                    n_draft += int(dm.sum())
                    n_acc += int(am.sum())
                    n_rej += int(rm.sum())
                ck.check(ok_sets,
                         f'{asset} review per-row set relations '
                         '(draft == accepted|rejected, accepted == final, '
                         'disjoint, valid reasons)')
                ck.check(n_draft == n_acc + n_rej,
                         f'{asset} review totals consistent '
                         f'({n_draft} vs {n_acc}+{n_rej})')
                ck.check(review.get('draft_px') == n_draft
                         and review.get('accepted_px') == n_acc
                         and review.get('rejected_px') == n_rej,
                         f'{asset} review metadata == recomputed totals')
    ...
    # ---- report / receipt -------------------------------------------
    rep_p = output / 'NIGHT03_PATCH2B_R1A_R5_REPORT.md'
    rec_p = output / 'NIGHT03_PATCH2B_R1A_R5_RECEIPT.txt'
    if ck.check(rep_p.exists(), 'report present'):
        ...
        ck.check(f'STATUS = {STATUS_EXPECTED}' in rep_lines,
                 'report STATUS R1A-R5')
        ck.check('PATCH2B_R1A_R4' not in rep, 'report carries no R1A-R4 name')
        ck.check('PATCH2B_R1A_R3' not in rep, 'report carries no R1A-R3 name')
        # split report into '## ' sections so per-asset keys are scoped
        sections = []
        ...
        for asset in ('a01', 'a05'):
            ...
            for k, v in tail_stats[asset].items():
                ...
                ck.check(not conflicts,
                         f'report {asset} {k} has no conflicting line '
                         f'({conflicts[:1]})')
        ...
        for cname in CLASSES:
            ...
            conflicts = [ln for ln in a16_sec
                         ...
                         and ln != exp]
            ck.check(not conflicts,
                     f'report a16 class {cname} has no conflicting line '
                     f'({conflicts[:1]})')
    if ck.check(rec_p.exists(), 'receipt present'):
        rec = rec_p.read_text(encoding='utf-8')
        ck.check(f'STATUS = {STATUS_EXPECTED}' in rec,
                 'receipt STATUS R1A-R5')
        ck.check('PATCH2B_R1A_R4' not in rec,
                 'receipt carries no R1A-R4 name')
