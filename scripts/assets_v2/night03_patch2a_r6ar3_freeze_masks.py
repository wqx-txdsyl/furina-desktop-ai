"""NIGHT-03 Recovery Patch 2A R6A-R3 — a16 ownership Gate (Builder).
...
OUT = ROOT / 'data/assets_v2/repair_candidates/night03_patch2a_r6ar3'
...
def main():
    ...
    assert doc['format'] == 'night03-ownership-scanline-rle-v3'
    ...
    # ---- frozen occlusion ledger verification (no tautology: the ledger
    # pixels are checked one by one against the rebuilt ownership) ---------
    for entry in ledger:
        winner = int(entry['winner'])
        px_list = entry.get('pixels') or []
        assert len(px_list) == int(entry['px']), (
            'ledger px count mismatch for bbox '
            f"{entry['bbox']}")
        for pix in px_list:
            ...
            assert ownership[y, x] == winner, (
                ...
                f'!= winner {winner}')
    assert ledger_px_total == sum(len(e.get('pixels') or []) for e in ledger)
    ...
    plan = {
        'task': 'NIGHT03_RECOVERY_PATCH2A_R6AR3_OWNERSHIP_GATE',
        'base_sha': BASE_SHA,
    ...
    (mask_dir / 'night03_patch2a_r6ar3_mask_plan.json').write_text(
    ...
    (OUT / 'NIGHT03_PATCH2A_R6AR3_REPORT.md').write_text(
        '\n'.join(lines), encoding='utf-8', newline='\n')

    print(f'R6A-R3 OK MASK_PLAN_SHA256={sha}')
