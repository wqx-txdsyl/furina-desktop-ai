"""NIGHT-03 Recovery Patch 2A R5 — single-source ownership map (Builder).
...
D16 = {
    ...
    'cane_parts': [
        {'name': 'grip_guard', 'box': (630, 940, 800, 1232),
        ...
        {'name': 'blade', 'box': (592, 1155, 722, 1450),
         'colors': ['steel_blue', 'silver', 'blade_highlight'],
         'seeds': [(660, 1260), (615, 1400), (640, 1200), (630, 1300), (618, 1400), (608, 1250)], 'fill': 'full'},
    ],
...
def colour_test(rgb, names):
    ...
    for nm in names:
        if nm == 'gold':
            ...
            out |= (b - r >= 65) & (b >= 110)
        elif nm == 'steel_blue':  # g-r steel-blue body (blade, not navy cloth)
            out |= (g - r >= 20) & (b - r >= 10) & (bright <= 235)
...
def build_protected_masks(rgb, alpha, d16):
    ...
    glove = glove | (ndimage.binary_fill_holes(glove) & alpha
                     & box_mask((h, w), [p['glove_zone']])
                     & (colour_test(rgb, ['dark_fabric']) & alpha))
...
def freeze_r1_a01_a05(asset, im):
    """Byte-identical re-run of the accepted R3/R4 a01/a05 pipeline."""
    h, w = im.shape[:2]
    rgb = im[..., :3].astype(int)
    alpha = im[..., 3] > 8
    d = DECLARED_R1[asset]
...
def freeze_a16(im):
    ...
    # ---- 0. R5 OWNERSHIP MAP (single source of truth) ----
    # labels: 0 other/background, 1 protected-hair, 2 protected-quill-paper,
    # 3 protected-glove-cuff, 4 protected-costume, 5 remove-cane,
    # 6 remove-chair
    ownership = np.zeros((h, w), np.uint8)
    ownership[alpha] = 7          # provisional: unassigned-alpha
    ownership[~alpha] = 0

    # ---- 1. protected masks FIRST (inclusion-only), then frozen ----
    prot = build_protected_masks(rgb, alpha, D16)
    ownership[prot['protected_existing_hair']] = 1
    ownership[prot['protected_quill_paper']] = 2
    ownership[prot['protected_glove_gold_cuff']] = 3
    ownership[prot['protected_costume']] = 4
    ...
    dest = np.fliplr(tail_source) & ~alpha
    ownership[tail_source] = 8   # remove-tail (declared repair target)
    seam = np.zeros((h, w), bool)
    near = ndimage.binary_dilation(alpha, ST8, iterations=6)
    ...
    ownership[cane] = 5
    ownership[chair] = 6
    ownership[cleanup] = 9   # speck cleanup: allowed term
    # hand reconstruction band: label 9 (repair-hand), an allowed term
    hand = np.zeros((h, w), bool)
    hx0, hy0, hx1, hy1 = D16['hand_band']
    hand[hy0:hy1, hx0:hx1] = (alpha[hy0:hy1, hx0:hx1]
                              & (br16[hy0:hy1, hx0:hx1] >= 130)
                              & (r_[hy0:hy1, hx0:hx1] >= 120)
                              & (r_[hy0:hy1, hx0:hx1] > g_[hy0:hy1, hx0:hx1])
                              & (g_[hy0:hy1, hx0:hx1] > b_[hy0:hy1, hx0:hx1])
                              & (r_[hy0:hy1, hx0:hx1] - b_[hy0:hy1, hx0:hx1] >= 30))
    hand &= ~box_mask((h, w), [(735, 910, 812, 1015)])
    ownership[hand] = 9
    # safe default: remaining alpha pixels belong to the character body
    ownership[(ownership == 7) & alpha] = 4
    ...
    # ---- R5 adjudication: unowned-alpha gap regions, declared from the
    # manual review of the corridor crops; applied ONLY to pixels still
    # unassigned (7), never overriding protected/cane/chair labels ----
    def gapfill(label, box, cond):
        x0, y0, x1, y1 = box
        sub = ownership[y0:y1, x0:x1]
        rs, gs, bs = r_[y0:y1, x0:x1], g_[y0:y1, x0:x1], b_[y0:y1, x0:x1]
        brs = br16[y0:y1, x0:x1]
        sel = sub == 7
        if cond == 'any':
            pass
        elif cond == 'blue':
            sel &= (gs - rs >= 15) & (bs >= gs - 5)
        elif cond == 'warm':
            sel &= (rs >= gs) & (rs - bs >= 8) & (brs <= 235)
        elif cond == 'dark_navy':
            sel &= (bs >= gs) & (brs <= 150)
        elif cond == 'bright':
            sel &= (brs >= 165) & (abs(rs - gs) <= 40)
        sub[sel] = label

    # cane: pommel assembly + collar/cage + staff strips (visual review)
    gapfill(5, (735, 916, 808, 1010), 'any')      # white ball
    gapfill(5, (748, 902, 806, 968), 'any')       # gold crown
    gapfill(5, (686, 998, 764, 1095), 'any')      # collar
    gapfill(5, (644, 1035, 750, 1170), 'any')     # ornate cage
    gapfill(5, (604, 1075, 718, 1170), 'any')     # staff top strips
    # chair: skeleton outline fragments (warm dark lines)
    gapfill(6, (410, 1080, 730, 1470), 'warm')
    # costume: sash/coattail right band + leftovers
    gapfill(4, (695, 1050, 850, 1420), 'blue')
    gapfill(4, (595, 1075, 690, 1165), 'dark_navy')

    # masks derived BY EQUALITY from the ownership map
    cane = ownership == 5
    chair = ownership == 6
    prot['protected_existing_hair'] = ownership == 1
    prot['protected_quill_paper'] = ownership == 2
    prot['protected_glove_gold_cuff'] = ownership == 3
    prot['protected_costume'] = ownership == 4
    ...
    np.save(OUT / 'a16_ownership_map.npy', ownership)
...
def main():
    ...
    for asset, fn in ASSETS.items():
        ...
        # R5: ownership map is the single source — save it and audit
        if asset != 'a16':
            continue_audit = False
        own = (np.load(OUT / 'a16_ownership_map.npy')
               if asset == 'a16' else None)
        if own is not None:
            Image.fromarray((own * 30).clip(0, 255).astype(np.uint8),
                            'L').save(adir / 'a16_ownership_map.png')
        unassigned = int(((own == 7) & (im[..., 3] > 8)).sum())
        if asset == 'a16':
            ent['ownership_unassigned_alpha_px'] = unassigned
            if unassigned:
                fail.append(f'{asset}: {unassigned} alpha pixels unassigned '
                            f'in ownership map')
    ...
    im16 = np.array(Image.open(MASTERS / ASSETS['a16']).convert('RGBA'))
    a16dir = OUT / 'a16'
    a16dir.mkdir(exist_ok=True)
    groups = {
        'cane': ['a16_cane_removal.png'],
        'chair': ['a16_chair_removal.png'],
        'cane_chair': ['a16_cane_removal.png', 'a16_chair_removal.png'],
    }
    h16, w16 = im16.shape[:2]
    bgs = {
        'checker': checker(h16, w16).astype(np.float32),
        'dark': np.full((h16, w16, 3), 32, np.float32),
        'light': np.full((h16, w16, 3), 240, np.float32),
    }
    af16 = im16[..., 3:4].astype(np.float32) / 255.0
    regs = {'cane': (592, 1155, 722, 1450),
    ...
    for tag, files in groups.items():
        ...
        for bgn, bg in bgs.items():
            comp = (cut[..., :3].astype(np.float32) * af
                    + bg * (1 - af)).clip(0, 255).astype(np.uint8)
            Image.fromarray(comp).save(
                a16dir / f'a16_{tag}_cutout_{bgn}_100.png')
            for z in (200, 400):
                rx0, ry0, rx1, ry1 = regs[tag]
                sub = Image.fromarray(comp[ry0:ry1, rx0:rx1]).resize(
                    ((rx1 - rx0) * z // 100, (ry1 - ry0) * z // 100),
                    Image.NEAREST)
                sub.save(a16dir / f'a16_{tag}_cutout_{bgn}_{z}.png')

    # per-protected isolated cutouts: master with ONLY that protected region
    for pname in ('protected_existing_hair', 'protected_quill_paper',
                  'protected_glove_gold_cuff', 'protected_costume'):
        pm = np.array(Image.open(
            a16dir / f'a16_{pname}.png')) == 255
        iso = im16.copy()
        iso[..., 3][~pm] = 0
        ...
        Image.fromarray(comp).save(
            a16dir / f'a16_{pname}_isolated_cutout.png')
    ...
    # ---- R4: report + receipt are GENERATED from the frozen plan ----
    def px_of(asset, name):
        return plan['assets'][asset]['masks'][name]['px']

    a16m = plan['assets']['a16']['masks']
    rep = f'''# NIGHT03_PATCH2A_R4_MASK_FREEZE_REPORT — Complete Frozen Semantic Mask Gate

- patch_id: NIGHT03_RECOVERY_PATCH2A_R5
- base_sha: {plan['base_sha']}
- single_source: a16_ownership_map.png (labels 0-6, mutual exclusive)
...
| asset | mask | px |
|---|---|---:|
| a01 | tail_source | {px_of('a01', 'tail_source')} |
| a01 | tail_destination | {px_of('a01', 'tail_destination')} |
| a01 | seam_reconstruction | {px_of('a01', 'seam_reconstruction')} |
| a01 | authorized_cleanup | {px_of('a01', 'authorized_cleanup')} |
| a01 | protected_identity | {px_of('a01', 'protected_identity')} |
| a01 | protected_costume | {px_of('a01', 'protected_costume')} |
| a01 | protected_props | {px_of('a01', 'protected_props')} |
| a01 | allowed_edit | {px_of('a01', 'allowed_edit')} |
| a05 | tail_source | {px_of('a05', 'tail_source')} |
| a05 | tail_destination | {px_of('a05', 'tail_destination')} |
| a05 | seam_reconstruction | {px_of('a05', 'seam_reconstruction')} |
| a05 | authorized_cleanup | {px_of('a05', 'authorized_cleanup')} |
| a05 | protected_identity | {px_of('a05', 'protected_identity')} |
| a05 | protected_costume | {px_of('a05', 'protected_costume')} |
| a05 | protected_props | {px_of('a05', 'protected_props')} |
| a05 | allowed_edit | {px_of('a05', 'allowed_edit')} |
| a16 | tail_source | {px_of('a16', 'tail_source')} |
| a16 | tail_destination | {px_of('a16', 'tail_destination')} |
| a16 | seam_reconstruction | {px_of('a16', 'seam_reconstruction')} |
| a16 | speck_cleanup | {px_of('a16', 'speck_cleanup')} |
| a16 | chair_removal | {px_of('a16', 'chair_removal')} |
| a16 | cane_removal | {px_of('a16', 'cane_removal')} |
| a16 | hair_reconstruction | {px_of('a16', 'hair_reconstruction')} |
| a16 | hand_reconstruction | {px_of('a16', 'hand_reconstruction')} |
| a16 | protected_existing_hair | {px_of('a16', 'protected_existing_hair')} |
| a16 | protected_quill_paper | {px_of('a16', 'protected_quill_paper')} |
| a16 | protected_glove_gold_cuff | {px_of('a16', 'protected_glove_gold_cuff')} |
| a16 | protected_costume | {px_of('a16', 'protected_costume')} |
| a16 | allowed_edit | {px_of('a16', 'allowed_edit')} |

## verification

- verifier VERIFY OK; residual total == sum(classes) for a01 ({plan['assets']['a01']['boundary_residual_px']});
...
OWNERSHIP_UNASSIGNED_ALPHA_PX = {plan['assets']['a16']['ownership_unassigned_alpha_px']}
A16_SEMANTIC_GATE = PENDING_INDEPENDENT_REVIEW
STATUS = READY_FOR_NIGHT03_PATCH2A_R5_OWNERSHIP_REVIEW
'''
    (OUT.parent / 'NIGHT03_PATCH2A_R5_MASK_FREEZE_REPORT.md').write_text(
        rep, encoding='utf-8')
...
GENERATION_CALLS = 0
A01_MASKS_BYTE_IDENTICAL_TO_R3 = true
A05_MASKS_BYTE_IDENTICAL = true
A01_RESIDUAL_TOTAL = {plan['assets']['a01']['boundary_residual_px']}
A01_RESIDUAL_CLASSIFIED = {sum(plan['assets']['a01']['residual_class_px'].values())}
PROTECTED_MASKS_PREDECLARED_GEOMETRIC_PARTITION = true
...
REPORT = data/assets_v2/repair_candidates/night03_patch2a_r4/NIGHT03_PATCH2A_R5_MASK_FREEZE_REPORT.md
STATUS = READY_FOR_NIGHT03_PATCH2A_R5_OWNERSHIP_REVIEW'''
    (OUT.parent / 'NIGHT03_PATCH2A_R5_RECEIPT.txt').write_text(
