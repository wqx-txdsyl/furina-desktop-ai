"""NIGHT-03 Recovery Patch 2A R2 — complete frozen semantic masks (Builder).
...
D16 = {
    ...
    'prot': {
        ...
        'items': [
            {'name': 'dress', 'box': (380, 880, 680, 1240),
            ...
            {'name': 'coattail_r', 'box': (695, 1050, 850, 1400),
             'class': 'dark_fabric', 'seed': (760, 1200)},
            {'name': 'sash', 'box': (695, 1140, 830, 1270),
    ...
    'cane_parts': [
        {'name': 'grip_guard', 'box': (620, 1000, 782, 1232),
         ...
         'seeds': [(725, 1065), (672, 1185)],
         'fill': 'holes',          # enclosed dark inlays join via fill
         },
...
def build_protected_masks(rgb, alpha, d16):
    ...
    # costume: positive per-item components
    costume = np.zeros_like(alpha)
    for item in p['items']:
        zone = box_mask((h, w), [item['box']])
        costume |= all_comps(colour_test(rgb, [item['class']]) & alpha
                             & zone, 100)
...
def freeze_a16(im):
    ...
    # interior grain, outline and AA: fill enclosed holes, then add the
    # dark-warm boundary band hugging the components (never near protected)
    filled = ndimage.binary_fill_holes(chair) & ch_zone & alpha
    ...
    chair = ndimage.binary_fill_holes(chair) & ch_zone & alpha & warm
    ...
    for part in D16['cane_parts']:
        ...
        else:  # 'none': grip — enclosed dark pixels are glove fingers
            cane |= comp & alpha
    # blue-dark outline band hugging the cane (never near protected, never
    # inside the ribbon box, never warm chair pixels: r>=g excluded)
    b_band = (ndimage.binary_dilation(cane, ST8, iterations=5) & ~cane
              & box_mask((h, w), [D16['cane_parts'][2]['box']])
              & alpha & (br16 <= 130)
