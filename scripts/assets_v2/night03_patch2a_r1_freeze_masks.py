"""NIGHT-03 Recovery Patch 2A R1 — corrected frozen semantic masks (Builder).

Task book: NIGHT03_RECOVERY_PATCH2A_R1 (Reviewer: ChatGPT Codex).
BASE_SHA: e8311a25c7a89059c75c8f19aa9b1837e4b719e2 (Patch 2A).  Scope: a01 / a05 / a16 only.
R1 fixes: a01 lower-curl inclusion; a16 cane ribbon false component removed;
a16 chair restricted to three identified semantic components; ALL protected
masks constructed independently from declared zones + disjoint colour classes
(never by subtracting removal masks).  a05 pipeline is byte-identical to 2A.
...
OUT = ROOT / 'data/assets_v2/repair_candidates/night03_patch2a_r1/mask_plan'
OLD_MASK_PLAN = ROOT / 'data/assets_v2/repair_candidates/night03_patch2/mask_plan'
CANVAS = (1024, 1536)
...
DECLARED = {
    'a01': {
        ...
        'props_cand_exclude_box': (283, 998, 352, 1470),
        'props_parts': [
    ...
    'a16': {
        ...
        'cane_parts': [
            {'box': (620, 1000, 780, 1230), 'colors': ['gold'],
             'seeds': [(725, 1065), (672, 1185)], 'fill': False},
            {'box': (735, 910, 812, 1015), 'colors': ['white', 'gold'],
             'seeds': [(770, 955)], 'fill': True},
            {'box': (595, 1160, 720, 1445), 'colors': ['deep_blue'],
             'seeds': [(660, 1260), (615, 1400)], 'fill': False},
        ],
        # theReviewer-located false ribbon component at (622,1110)-(690,1155)
        # (seed (675,1140)) is structurally excluded: no cane part covers it.
        ...
        'chair_body_guard': (475, 1190, 625, 1252),
        'chair_seeds': [(600, 1275), (594, 1350), (677, 1300)],
        'chair_expected_comps': 3,
        'fist_box': (620, 940, 775, 1135),
        'hand_band': (715, 1035, 750, 1148),
        'glove_zone': (600, 1000, 760, 1160),
        'cuff_gold_zone': (620, 1000, 709, 1130),
...
def colour_test(rgb, names):
    ...
    for n in names:
        ...
        elif n == 'deep_blue':  # a16 blade core (AA fringe stays costume-side)
            out |= (b - r >= 65) & (b >= 110)
...
def freeze(asset, im):
    ...
    br_ = b_ - r_
    dark_any = (br16 < 130) & (br_ >= 10) & (br_ <= 60)
    white_c = (br16 >= 185) & (np.abs(br_) <= 25)
    blade_blue = (br_ >= 65) & (b_ >= 110)
    gold_c = ((r_ >= 120) & (r_ > g_) & (g_ > b_)
              & (r_ - b_ >= 30))
    shaft_c = (br_ >= 60) & (b_ >= 140) & (br16 < 210)
    dark_c = (br16 < 130) & (br_ <= 60) & (b_ > g_) & alpha
    wood_tight = (((r_ > g_) & (g_ > b_) & (r_ - b_ >= 14)
                   & (r_ - g_ <= 50) & (r_ >= 40) & (r_ < 210)
                   & (br16 <= 130)) & alpha)
    ...
    hard_guard = identity | costume | props | extra   # shell-level guards
    shell = (shell_zone & alpha & corridor & ~tail_solid & ~hard_guard)
    # core pixels are seed-verified tail semantics: only true protected
    # components (never the wide guard boxes) may subtract them.
    tail_source = (tail_solid | shell) & alpha & ~hard_guard
    # a01 only: the seed-verified core may keep pixels inside the wide props
    # guard box when they belong to the tail component (props component is
    # what actually protects cane pixels)
    if asset == 'a01':
        tail_source |= (tail_solid & alpha & ~props & ~extra
                        & ~(identity | costume))
    ...
    if asset == 'a16':
        ...
        # chair: dark-wood components inside the declared corridor, minus
        # her body band and the shoe/sock zone
        wood = (wood_tight
                & box_mask((h, w), [d['chair_corridor']])
                & ~box_mask((h, w), [d['chair_shoe_guard']])
                & ~box_mask((h, w), [d['chair_body_guard']]))
        # only the three identified chair components (seat + two legs);
        # every other same-coloured fragment is NOT chair and stays out.
        chair_parts = []
        labs, nlab = ndimage.label(wood, structure=ST8)
        seen = np.zeros(nlab + 1, bool)
        for sx, sy in d['chair_seeds']:
            ident = labs[sy, sx]
            assert ident, f'chair seed ({sx},{sy}) empty'
            assert not seen[ident], 'chair seed hit an already-taken comp'
            comp = labs == ident
            seen[ident] = True
            chair_parts.append(comp)
        chair = np.zeros_like(wood)
        for comp in chair_parts:
            # keep only true dark-wood pixels (plus enclosed wood holes):
            # never costume-coloured AA or blue-dominant blade pixels
            filled = ndimage.binary_fill_holes(comp) & wood_tight
            filled &= box_mask((h, w), [d['chair_corridor']])
            chair |= filled
        chair &= alpha
        ...
        for part in d['cane_parts']:
            ...
            cand = colour_test(rgb, part['colors']) & alpha & zone
            labp, _ = ndimage.label(cand, structure=ST8)
            for sx, sy in part['seeds']:
                comp = labp == labp[sy, sx]
                if part.get('fill'):
                    # pommel/blade: enclosed holes travel with the part
                    cane |= ndimage.binary_fill_holes(comp) & alpha
                else:
                    # gold grip/guard: component pixels only — the enclosed
                    # dark pixels are her glove fingers and stay protected
                    cane |= comp & alpha
        cane &= alpha
        ...
        # ---- independent protected masks (R1): declared zones + disjoint
        # colour classes only; removal masks are NEVER referenced.
        blade_zone = box_mask((h, w), [(595, 1160, 720, 1445)])
        cane_zone = box_mask((h, w), [p['box'] for p in d['cane_parts'][:2]])
        pommel_zone = box_mask((h, w), [(735, 910, 812, 1015)])
        chair_zone = box_mask((h, w), [d['chair_corridor']])
        exclusion = ((wood_tight & chair_zone) | (blade_blue & blade_zone)
                     | ((gold_c | white_c | shaft_c) & cane_zone))
        out['protected_existing_hair'] = identity & ~exclusion
        out['protected_quill_paper'] = extra & ~exclusion
        glove_px = ((dark_c & box_mask((h, w), [d['glove_zone']]))
                    | (gold_c & box_mask((h, w), [d['cuff_gold_zone']]) & alpha)
                    | (white_c & box_mask((h, w), [d['frill_zone']]) & alpha))
        glove_px = glove_px & ~exclusion
        out['protected_glove_gold_cuff'] = glove_px
        out['protected_costume'] = (costume & ~exclusion
                                    & ~box_mask((h, w), d['cleanup_boxes']))
        # hand_reconstruction: independently declared bright band between the
        # fingers (grip/blade pixels that become holes once the cane is gone);
        # excludes pommel, ribbon, cuff gold and every transparent pixel.
        hx0, hy0, hx1, hy1 = d['hand_band']
        hand_recon = (alpha[hy0:hy1, hx0:hx1]
                      & gold_c[hy0:hy1, hx0:hx1])
        hr = np.zeros((h, w), bool)
        hr[hy0:hy1, hx0:hx1] = hand_recon
        hr &= ~pommel_zone & ~box_mask((h, w), [d['ribbon_false_box']])
        hr &= ~box_mask((h, w), [d['cuff_gold_zone']])
        hr &= alpha
        out['hand_reconstruction'] = hr
        raw_qp = int((chair & extra).sum())
        raw_glove = int(((cane & box_mask((h, w), [d['glove_zone']])
                          & alpha)).sum())
        out['_raw_qp'] = raw_qp
        out['_raw_glove'] = raw_glove
        out['_chair_parts'] = chair_parts
        allowed_terms = ['old_tail_removal', 'tail_destination',
...
def main():
    ...
    plan = {'task': 'NIGHT03_RECOVERY_PATCH2A_R1_MASK_FREEZE',
            'base_sha': 'e8311a25c7a89059c75c8f19aa9b1837e4b719e2',
            'prev_plan_sha256': 'aa366b183f6c5db1e92edcfb4145699589c6595e4481bc1720c511d7bf30aab6',
    ...
    for asset, fn in ASSETS.items():
        ...
        raw_qp = masks.pop('_raw_qp', None)
        raw_glove = masks.pop('_raw_glove', None)
        chair_parts = masks.pop('_chair_parts', None)
        adir = OUT / asset
        ...
        prot = [n for n in order
                if n.startswith('protected_') and n in masks]
        for pn in prot:
        ...
        if raw_qp is not None:
            ent['raw_zone_overlaps'] = {
                'chair_raw_quill_paper_box_px': raw_qp,
                'cane_raw_glove_zone_px': raw_glove,
                'allowed_cane_hand_internal_overlap':
                    ent['intersections'].get('cane_removal^hand_reconstruction'),
                'chair_component_count': int(len(chair_parts))}
        ...
        if asset == 'a01':
            ys_t = np.where(masks['tail_source'])[0]
            ent['tail_source_ymax'] = int(ys_t.max())
            if ys_t.max() < dcl['lower_curl_min_ymax']:
                fail.append(f'{asset}: lower curl still clipped '
                            f'(ymax={ys_t.max()})')
            # triptych: old (e8311a2) tail_source | new | residual classes
            old_p = OLD_MASK_PLAN / 'a01' / 'a01_tail_source.png'
            old_m = np.array(Image.open(old_p)) == 255
            resid = tint_all & ~masks['tail_source'] & (im[..., 3] > 8)
            ...
            for i in range(1, rn + 1):
                ...
                if px < 8:
                    continue
                ys, xs = np.where(c)
                bb = (int(xs.min()), int(ys.min()), int(xs.max()),
                      int(ys.max()))
                if bb[0] >= 302:
            ...
            if any(t['class'] == 'unclassified' for t in tab):
                fail.append(f'{asset}: unclassified residual components '
                            'remain (see residual_components)')
            # classified residual may stay; unclassified must be zero
            tri = np.zeros((1536, 3 * 1024 + 40, 3), np.uint8)
            for k, mm in enumerate((old_m, masks['tail_source'],
                                    resid)):
                reg = tri[:, k * (1024 + 20):k * (1024 + 20) + 1024]
                base = (im[..., :3].astype(np.float32)
                        * (im[..., 3:4] / 255.0)
                        + 255 * (1 - im[..., 3:4] / 255.0))
                reg[:] = base.clip(0, 255).astype(np.uint8)
                if k < 2:
                    col = (255, 60, 60) if k == 0 else (60, 220, 60)
                    reg[mm] = col
                else:
                    cls_col = {'lining_boundary_AA': (150, 150, 255),
                               'upper_fringe': (255, 200, 80),
                               'cane_boundary_AA': (180, 120, 255),
                               'unclassified': (255, 0, 0)}
                    for t in tab:
                        bx0, by0, bx1, by1 = t['bbox']
                        sub = (rl[by0:by1 + 1, bx0:bx1 + 1]
                               == rl[by0 + (by1 - by0) // 2 + 1
                                     if False else by0, bx0] ) if False else None
                    for i in range(1, rn + 1):
                        c = rl == i
                        if not (c & resid).any() or c.sum() < 8:
                            continue
                        ys, xs = np.where(c)
                        bb = (int(xs.min()), int(ys.min()))
                        for t in tab:
                            if t['bbox'][0] == bb[0] and t['bbox'][1] == bb[1]:
                                reg[c] = cls_col[t['class']]
                                break
            Image.fromarray(tri).save(
                adir / 'a01_old_new_residual_triptych.png')
