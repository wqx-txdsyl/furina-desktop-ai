"""NIGHT-03 Recovery Patch 2A R6A-R3 — Builder manual annotation tool.
...
OUTDIR = ROOT / 'data/assets_v2/repair_candidates/night03_patch2a_r6ar3'
...
OCCLUSION_RESOLUTION = [
    {'bbox': [634, 1216, 686, 1272],
     'winner': 5,
     'rationale': 'steel blade passes in front of the wooden seat board '
                  '(seat top-right corner continues behind the blade)'},
    {'bbox': [656, 1258, 688, 1310],
     'winner': 5,
     'rationale': 'steel blade passes in front of the right chair leg top'},
    {'bbox': [601, 1375, 618, 1403],
     'winner': 5,
     'rationale': 'spearhead left contour over the left chair leg edge; '
                  'visible brown pixels are excluded from the spearhead '
                  'primitive and remain in the chair rows'},
    {'bbox': [590, 1404, 604, 1436],
     'winner': 5,
     'rationale': 'spearhead left contour over the narrowing left chair '
                  'leg below the leg tip; visible brown pixels excluded '
                  'as above'},
    {'bbox': [602, 1430, 620, 1452],
     'winner': 5,
     'rationale': 'spearhead blade over the left chair leg foot; the '
                  'foot brown pixels are excluded from the spearhead '
                  'primitive and remain in the chair rows'},
]
OCCLUSION_RATIONALE_EXTRA = {
    6: 'brown chair-leg pixels inside the leaf bounding box belong to the '
       'left chair leg; the spearhead primitive excludes them by '
       'construction (colour-resolved rows), so no cane winner exists '
       'there',
}
...
def main():
    ...
    # ---- resolve overlap through the frozen ledger ------------------------
    ys, xs = np.where(prim_overlap)
    resolved = np.zeros((h, w), np.uint8)          # winner label per overlap px
    covered = np.zeros((h, w), bool)
    ledger = []
    for entry in OCCLUSION_RESOLUTION:
        x0, y0, x1, y1 = entry['bbox']
        sel = prim_overlap[y0:y1 + 1, x0:x1 + 1]
        px = np.zeros((h, w), bool)
        px[y0:y1 + 1, x0:x1 + 1] = sel
        n = int(px.sum())
        ledger.append({'bbox': entry['bbox'], 'px': n,
                       'winner': entry['winner'],
                       'rationale': entry['rationale'],
                       'pixels': [[int(x), int(y)] for y, x in
                                  zip(*np.where(px))]})
        resolved[px] = entry['winner']
        covered |= px
    uncovered_overlap = prim_overlap & ~covered
    if int(uncovered_overlap.sum()):
        uys, uxs = np.where(uncovered_overlap)
        pts = sorted({(int(y), int(x)) for y, x in zip(uys, uxs)})[:40]
        raise SystemExit(f'unresolved primitive overlap: '
                         f'{int(uncovered_overlap.sum())} px at {pts}')

    # ---- paint: protect -> cane/chair non-overlap -> overlap winners -----
    ...
    doc = {
        'format': 'night03-ownership-scanline-rle-v3',
        'asset': 'a16',
