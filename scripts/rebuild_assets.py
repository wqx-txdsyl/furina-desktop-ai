...
用法：  python scripts/rebuild_assets.py [--dry-run] [--phase 1|2|3|all]
"""
...
log = get_logger("scripts.rebuild")
...
def build_sequences(cfg, out_dir: Path) -> None:
    ...
    from scripts.gen_sequences import build_sequence, gen_loop, _load_centered, CANVAS_W, CANVAS_H, N_LOOP
...
def main() -> int:
    ...
    # 选批次
    from scripts.generate_assets import batch_phase1, batch_phase2, batch_phase3
    if args.phase == "1":
        specs = batch_phase1()
    elif args.phase == "2":
        ...
        from scripts.generate_assets import batch_flat100full
