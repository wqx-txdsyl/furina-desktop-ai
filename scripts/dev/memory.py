"""记忆 CLI —— 查看/添加/夜间巩固（legacy-plan/6 §32, §40）。

    python scripts/dev/memory.py list          # 列出记忆
    python scripts/dev/memory.py add "用户让我整理下载文件" --importance 0.9 --level episodic
    python scripts/dev/memory.py consolidate   # 夜间巩固（把近期事件概括为一条总结记忆）
    python scripts/dev/memory.py relationship  # 查看关系维度
"""
