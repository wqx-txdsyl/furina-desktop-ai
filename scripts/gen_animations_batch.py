"""多帧动画批量生成（plan/2 §7-L4, ⑦§19-21）。

在单脚本 gen_animation.py 基础上，为多个命名动画各生成一条多帧序列。
用法：
    python scripts/gen_animations_batch.py --name walk --kf a.png b.png ...
    python scripts/gen_animations_batch.py --jobs "eat:sit.png:eat.png" ...
从 plan/2/README 读取需求，逐条生成；每条出帧后立即入 manifest（断点续跑）。
...
log = get_logger("scripts.gen_animations_batch")
